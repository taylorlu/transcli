...
{
  public:
    Vector3<T>(T nx = 0, T ny = 0, T nz = 0)
    {
      x = nx;
      y = ny;
      z = nz;
    ...
    Vector3<T>(const Vector3<T> &source)
    {
      x = source.x;
      y = source.y;
      z = source.z;
    ...
    Vector3<T>(const Vector2<T> &source, const T Z){
      x = source.x;
      y = source.y;
      z = Z;
    }
    ...
    inline Vector3<T> operator + (const Vector3<T> &right){
      return Vector3(right.x + x, right.y + y, right.z + z);
    }
 
    inline Vector3<T> operator - (const Vector3<T> &right){
      return Vector3<T>(-right.x + x, -right.y + y, -right.z + z);
    }
    ...
    inline Vector3<T>  operator / (const T scale){
      return scale ? Vector3<T>(x/scale, y/scale, z/scale): Vector3<T>(0, 0, 0);
    }
    ...
    inline Vector3<T> &operator += (const T xyz)
    {
      x += xyz;
      y += xyz;
      z += xyz;
    ...
    inline void set(const Vector3<T> & t)
    {
      x = t.x;
      y = t.y;
      z = t.z;
    ...
    inline const T getDotProduct(const Vector3<T> &t) const
    {
    ...
    inline Vector3<T> &crossProduct(const Vector3<T> &t1, const Vector3<T> &t2)
    {
    ...
    inline const T getDistance(const Vector3<T> &v2)  const 
    {
    ...
    inline const T getAngle(const Vector3<T> &v2)  const 
    {             
    ...
    inline Vector3<T> &clamp(T min, T max)
    {
    ...
    friend ostream & operator << ( ostream & out, const Vector3<T> &right){
      return out << "Vector(" << right.x << ", " << right.y << ", " << right.z <<")";
    }

    friend istream & operator >> (istream & in, Vector3<T> &right){
