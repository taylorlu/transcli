...
class Matrix4
{
  public:
               ...
               T m12, T m13, T m14, T m15)
   {
    ...
    inline Matrix4<T> &lookAt(T viewX , T viewY , T viewZ ,
                              ...
                              T upX   , T upY   , T upZ   )
    {
      return lookAt(Vector3<T>(viewX , viewY , viewZ ),
                    Vector3<T>(focusX, focusY, focusZ),
                    Vector3<T>(upX   , upY   , upZ   ));
    }

    inline Matrix4<T> &lookAt(Vector3<T> &viewPosition,
                              Vector3<T> &viewFocus, 
                              Vector3<T> &upVector)
    {
      Vector3<T> *xaxis = (Vector3<T> *)m ,
                *up    = (Vector3<T> *)(m + 4),
                *at    = (Vector3<T> *)(m + 8);
    ...
    inline friend Vector3<T> operator * (const Vector3<T> &t,const Matrix4<T> &mat){
      return Vector3<T>(mat.m[ 0]*t.x + mat.m[ 4]*t.y + mat.m[ 8]*t.z + mat.m[12],
                        mat.m[ 1]*t.x + mat.m[ 5]*t.y + mat.m[ 9]*t.z + mat.m[13],
                        mat.m[ 2]*t.x + mat.m[ 6]*t.y + mat.m[10]*t.z + mat.m[14]);
    }

    inline friend Vector3<T> operator * (const Matrix4<T> &mat,const Vector3<T> &t){
      return Vector3<T>(mat.m[ 0]*t.x + mat.m[ 4]*t.y + mat.m[ 8]*t.z + mat.m[12],
                        mat.m[ 1]*t.x + mat.m[ 5]*t.y + mat.m[ 9]*t.z + mat.m[13],
                        mat.m[ 2]*t.x + mat.m[ 6]*t.y + mat.m[10]*t.z + mat.m[14]);
    }

    inline friend Vector4<T> operator * (const Matrix4<T> &mat, const Vector4<T> &t){
      return Vector4<T>(mat.m[ 0]*t.x + mat.m[ 4]*t.y + mat.m[ 8]*t.z + mat.m[12]*t.w,
    ...
    inline friend Vector4<T> operator * (const Vector4<T> &t,const Matrix4<T> &mat){
      return Vector4<T>(mat.m[ 0]*t.x + mat.m[ 4]*t.y + mat.m[ 8]*t.z + mat.m[12]*t.w,
                        mat.m[ 1]*t.x + mat.m[ 5]*t.y + mat.m[ 9]*t.z + mat.m[13]*t.w,
                        mat.m[ 2]*t.x + mat.m[ 6]*t.y + mat.m[10]*t.z + mat.m[14]*t.w,
                        mat.m[ 3]*t.x + mat.m[ 7]*t.y + mat.m[11]*t.z + mat.m[15]*t.w);
    ...
    inline friend  void operator *=(Vector3<T> &t,const Matrix4<T> &mat){
      t.set(mat.m[ 0]*t.x + mat.m[ 4]*t.y + mat.m[ 8]*t.z + mat.m[12],
            mat.m[ 1]*t.x + mat.m[ 5]*t.y + mat.m[ 9]*t.z + mat.m[13],
            mat.m[ 2]*t.x + mat.m[ 6]*t.y + mat.m[10]*t.z + mat.m[14]);
    }

    inline friend void operator *=(Vector4<T> &t,const Matrix4<T> &mat){
  ...
  __forceinline bool setInverse()
  {
    ...
    inline void setScales(const Vector3<T> &axis) 
    {
	    setIdentity();
      m[ 0] = axis.x;
      m[ 5] = axis.y;
    ...
    inline void rotateAxis(float angle, const Vector3<T> &axis) {
    ...
    inline void rotateY(const float angle){
      ...
      m[ 0] =  fastCos(angle);
      m[ 8] =  fastSin(angle);
      m[ 2] = -m[8];				//Exchange the minus of m[2] and m[8] (change)
      m[10] =  m[0];
    }  
    ...
    inline void rotateXYZ(const Vector3<T> &t)
    {
    ...
    void setTranslations(const Vector3<T> &t)
    {
      setIdentity();
      m[12] =  t.x;
      m[13] =  t.y;
