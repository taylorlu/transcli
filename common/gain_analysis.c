...
AnalyzeSamples (void* gainContext, const Float_t* left_samples, const Float_t* right_samples, size_t num_samples, int num_channels )
{
    ...
    while ( batchsamples > 0 ) {
        ...
        {   
            ...
            rg->lrsum = _mm_add_pd(rg->lrsum, __temp);
        }
        i = cursamples / 16;
        while (i--)
        {   
        ...
        if ( rg->totsamp == rg->sampleWindow ) {  // Get the Root Mean Square (RMS) for this set of samples
            ...
            int ival;
#ifdef HAVE_SSE2
            _mm_store_pd (__temp2, rg->lrsum);

            val = STEPS_per_dB * 10. * log10 ( (__temp2[0]+__temp2[1]) / rg->totsamp * 0.5 + 1.e-37 );
#else
            val  = STEPS_per_dB * 10. * log10 ( (rg->lsum+rg->rsum) / rg->totsamp * 0.5 + 1.e-37 );
#endif
            ival = (int) val;
            if ( ival <                     0 ) ival = 0;
